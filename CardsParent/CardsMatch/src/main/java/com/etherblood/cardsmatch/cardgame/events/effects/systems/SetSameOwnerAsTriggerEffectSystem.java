...
public class SetSameOwnerAsTriggerEffectSystem extends AbstractMatchSystem<EffectEvent> {
    ...
    public EffectEvent handle(EffectEvent event) {
        ...
        if(ownerFilterComponent != null) {
            ...
            EntityId owner = ownerFilterComponent.filter.select(data, event.effect, data.get(trigger, OwnerComponent.class).player).get(0);//data.get(trigger, OwnerComponent.class).player;
